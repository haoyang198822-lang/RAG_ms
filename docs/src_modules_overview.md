...
1. __PDF解析__：`pdf_parsing.py` → 结构化JSON
2. __表格序列化（可选）__：`tables_serialization.py`
3. __报告规整__：`parsed_reports_merging.py` → 规整为页文本
4. __文本分块__：`text_splitter.py`
5. __向量/检索库构建__：`ingestion.py`
6. __问题处理与RAG问答__：`questions_processing.py` + `retrieval.py` + `api_requests.py`
7. __检索重排__：`reranking.py`
8. __主流程调度__：`pipeline.py`
