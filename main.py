...
@cli.command()
def download_models():
    """No-op: docling parsing has been removed in favor of MinerU pipeline."""
    click.echo("已剔除 Docling 初始化/模型下载环节；请使用 MinerU 解析链路。")
...
def parse_pdfs(parallel, chunk_size, max_workers):
    raise click.ClickException("已剔除 Docling PDF 解析链路；请使用 MinerU 解析链路。")
