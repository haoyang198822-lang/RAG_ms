...
class VectorRetriever:
    ...
    def retrieve_by_company_name(self, company_name: str, query: str, llm_reranking_sample_size: int = None, top_n: int = 3, return_parent_pages: bool = False) -> List[Tuple[str, float]]:
        ...
        if expected_dim is not None and embedding_array.shape[1] != expected_dim:
            raise ValueError(
                f"Embedding维度({embedding_array.shape[1]})与向量库维度({expected_dim})不一致。"
                f"请设置环境变量 AGICTO_EMBEDDING_MODEL/OPENAI_EMBEDDING_MODEL 为与向量库一致的 embedding 模型。"
            )
...
class HybridRetriever:
    def __init__(
        ...
        self.reranker = LLMReranker(provider=reranking_provider)
