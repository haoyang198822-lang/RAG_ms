...
try:
    # Docling backend availability varies by installation.
    # Prefer the newer parser backend when available, otherwise fall back to pypdfium2.
    try:
        from docling.backend.docling_parse_v2_backend import DoclingParseV2DocumentBackend
    except Exception:
        from docling.backend.pypdfium2_backend import PyPdfiumDocumentBackend as DoclingParseV2DocumentBackend
    ...
    def __init__(
        ...
        if not _DOCLING_AVAILABLE:
            raise ModuleNotFoundError(
                "Missing optional dependency 'docling'. Install it to enable PDF parsing."
            )
