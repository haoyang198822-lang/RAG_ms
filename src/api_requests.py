...
class BaseIBMAPIProcessor:
    def __init__(self):
        ...
        self.base_url = os.getenv("AGICTO_BASE_URL", "https://api.agicto.cn/v1")
        self.default_model = os.getenv("AGICTO_DEFAULT_MODEL", "qwen-max-latest")
...
class BaseGeminiProcessor:
    def __init__(self):
        raise NotImplementedError("Gemini provider has been removed in favor of the agicto proxy.")
...
class APIProcessor:
    def __init__(self, provider: Literal["openai", "ibm", "gemini", "dashscope"] ="openai"):
        self.provider = provider.lower()
        if self.provider in {"openai", "ibm", "gemini"}:
